...
class IngestCrowdOxOrderSelections implements ShouldQueue
{
    ...
    {
        $ingestor = new CrowdOxOrderSelectionIngestor(null, $this->page_start, $this->pages_to_fetch);
        $ingest = $ingestor->ingest();

        if ($ingest->resourceExhausted() == false) {
            IngestCrowdOxOrderSelections::dispatch($this->page_start + $this->pages_to_fetch, $this->pages_to_fetch);
        }
    }
}
