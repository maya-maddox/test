...
class IngestCrowdOxOrderAddresses implements ShouldQueue
{
    ...
    {
        $ingestor = new CrowdOxOrderAddressIngestor(null, $this->page_start, $this->pages_to_fetch);
        $ingest = $ingestor->ingest();

        if ($ingest->resourceExhausted() == false) {
            IngestCrowdOxOrderAddresses::dispatch($this->page_start + $this->pages_to_fetch, $this->pages_to_fetch);
        }
    }
}
