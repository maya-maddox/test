...
class IngestCrowdOxOrders implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    protected $page_start = 1;
    protected $pages_to_fetch = 1;
    ...
    {
        $ingestor = new CrowdOxOrderRelationsIngestor(null, $this->page_start, $this->pages_to_fetch);
        $ingest = $ingestor->ingest();

        if ($ingest->resourceExhausted() == false) {
            IngestCrowdOxOrders::dispatch($this->page_start + $this->pages_to_fetch, $this->pages_to_fetch);
        }
    }
}
